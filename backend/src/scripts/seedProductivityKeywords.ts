...
import { Tenant } from "../modules/companies/Company.model";
...
const PRODUCTIVITY_KEYWORDS: Record<string, KeywordEntry[]> = {
  productive: [
    ...
    {
      keyword: "docs.google.com",
      type: "url",
      matchType: "contains",
      priority: 7,
    },
    {
      keyword: "sheets.google.com",
      type: "url",
      matchType: "contains",
      priority: 7,
    },
    {
      keyword: "slides.google.com",
      type: "url",
      matchType: "contains",
      priority: 6,
    },
  ...
  unproductive: [
    ...
    {
      keyword: "snapchat.com",
      type: "url",
      matchType: "contains",
      priority: 9,
    },
    {
      keyword: "pinterest.com",
      type: "url",
      matchType: "contains",
      priority: 8,
    },
...
async function seedProductivityKeywords() {
  try {
    ...
    if (tenants.length === 0) {
      console.log(
        "⚠️  No active tenants found. Run `npm run seed` first to create tenants."
      );
      process.exit(1);
    }
    ...
    for (const tenant of tenants) {
      ...
      const deleted = await ProductivityKeyword.deleteMany({
        tenantId: tenant._id,
      });
