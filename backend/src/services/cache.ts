  ...
  socket: {
    ...
    reconnectStrategy: (retries: number) => {
      if (retries > 5 && !isRedisAvailable) {
        ...
        logger.warn(
          "Redis not available after 5 attempts - running without cache. This is OK for development."
        );
...
redisClient.on("error", (_err: Error) => {
  ...
  if (!hasLoggedDisconnect) {
    ...
    logger.warn(
      "Redis unavailable - cache disabled. Start Redis to enable caching."
    );
  }
...
const connectRedis = async (): Promise<boolean> => {
  try {
    ...
    logger.warn(
      "Redis unavailable - running without cache. This is OK for development."
    );
    return false;
  }
};
...
export { connectRedis, isRedisConnected, cache };
