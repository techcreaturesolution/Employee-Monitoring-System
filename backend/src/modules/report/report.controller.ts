...
export const getAttendanceReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  res.json(new ApiResponse(200, 'Attendance report fetched.', data));
});

export const getProductivityReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.user?.tenantId;
  const { startDate, endDate, userId } = req.query;
  const data = await compileReportData('productivity', tenantId, startDate as string, endDate as string, userId as string);
  res.json(new ApiResponse(200, 'Productivity report fetched.', data));
});

export const getActivityReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.user?.tenantId;
  const { startDate, endDate, userId } = req.query;
  const data = await compileReportData('activity', tenantId, startDate as string, endDate as string, userId as string);
  res.json(new ApiResponse(200, 'Activity report fetched.', data));
});

export const getScreenshotReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.user?.tenantId;
  const { startDate, endDate, userId } = req.query;
  const data = await compileReportData('screenshots', tenantId, startDate as string, endDate as string, userId as string);
  res.json(new ApiResponse(200, 'Screenshot report fetched.', data));
});

export const exportExcel = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.user?.tenantId;
...
export const getEmployeeReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  res.json(new ApiResponse(200, 'Employee report fetched.', report));
});

export const getProjectReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.user?.tenantId;
  const tenantObjId = new mongoose.Types.ObjectId(String(tenantId));
  ...
  res.json(new ApiResponse(200, 'Project report fetched.', projects));
});

export const getTaskReport = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
  const tenantId = req.user?.tenantId;
  const tenantObjId = new mongoose.Types.ObjectId(String(tenantId));
  ...
  res.json(new ApiResponse(200, 'Task report fetched.', { taskStats, tasks }));
});
