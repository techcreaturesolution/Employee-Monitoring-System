...
const uploadWithRetry = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      logger.info(
        `[Attempt ${attempt}/${maxRetries}] Uploading to Cloudinary...`
      );
...
const uploadScreenshot = async (
  ...
  try {
    ...
    if (!file) {
      res
        .status(400)
        .json({ success: false, message: "No screenshot file provided." });
      return;
    }

    if (!isCloudinaryConfigured) {
      logger.warn(
        `[${requestId}] ⚠️  Cloudinary not configured – using local storage fallback.`
      );
    }
    ...
    if (isCloudinaryConfigured) {
      try {
        ...
        logger.info(
          `[${requestId}] Uploading to Cloudinary folder: ${customFolder}`
        );
        const cloudinaryResult = await uploadWithRetry(
          file.path,
          "screenshots",
          customFolder,
          3
        ...
        if (cloudinaryResult) {
          ...
          logger.info(
            `[${requestId}] ✅ Cloudinary upload successful: ${publicId}`
          );
        } else {
          cloudWarning =
            "Cloudinary returned empty response – using local storage.";
...
const getScreenshot = async (
  ...
  try {
    ...
    const screenshot = await Screenshot.findOne({ _id: id, tenantId }).populate(
      "userId",
  ...
  try {
    ...
    if (screenshot.publicId) {
      ...
      if (deleted) {
        ...
        logger.warn(
          `⚠️  Failed to delete from Cloudinary: ${screenshot.publicId}`
        );
      }
    }
    ...
    if (!screenshot.publicId) {
      ...
      const filePath = path.resolve(
        UPLOAD_DIR,
        path.basename(screenshot.imageUrl)
      );
...
export { uploadScreenshot, listScreenshots, getScreenshot, deleteScreenshot };
