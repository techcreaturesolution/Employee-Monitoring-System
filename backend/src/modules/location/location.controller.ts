...
import { LocationLog } from "./LocationLog.model";
...
const batchTrackLocations = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: `${docs.length} locations logged.` });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Batch location tracking failed.",
      error: (error as Error).message,
    });
  }
};
...
const getLiveLocations = async (
  ...
  try {
    ...
    const liveData = employees
      .filter(
        (e) => e.lastKnownLocation?.latitude && e.lastKnownLocation?.longitude
      )
      .map((e) => ({
        userId: e._id,
        name: e.name,
...
const getLocationTrail = async (
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to get location trail.",
      error: (error as Error).message,
    });
  }
};
