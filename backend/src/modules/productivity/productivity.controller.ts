...
import { ProductivityKeyword } from "./ProductivityKeyword.model";
...
async function classifyActivity(
  ...
  for (const kw of keywords) {
    ...
    if (matched)
      return kw.category as "productive" | "neutral" | "unproductive";
  }
    ...
    res.json({
      ...
      data: {
        ...
        stats: {
          ...
          total,
          productivePercent:
            total > 0 ? Math.round((productive / total) * 100) : 0,
          neutralPercent: total > 0 ? Math.round((neutral / total) * 100) : 0,
          unproductivePercent:
            total > 0 ? Math.round((unproductive / total) * 100) : 0,
        },
        topApps,
        topWebsites,
      },
...
// ============================================================
const getProductivityRange = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
const listKeywords = async (
  ...
  try {
    ...
    if (
      !["admin", "manager", "company_admin", "super_admin"].includes(
        req.user?.role || ""
      )
    ) {
      res
    ...
    const keywords = await ProductivityKeyword.find(filter).sort({
      category: 1,
      priority: -1,
    });
