...
const logActivity = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (uniqueDocs.length > 0) {
      ...
      res.status(201).json({
        success: true,
        message: `${logs.length} activities logged.`,
...
const getActivitySummary = async (
  ...
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate)
        (matchFilter.startTime as Record<string, unknown>).$gte = new Date(
          startDate as string
        );
      if (endDate)
        (matchFilter.startTime as Record<string, unknown>).$lte = new Date(
          endDate as string
        );
    }
...
export { logActivity, getActivityLogs, getActivitySummary };
