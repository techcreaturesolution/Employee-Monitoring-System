...
const punchOut = async (
  ...
  try {
    ...
    if (!attendance) {
      res.status(400).json({
        success: false,
        message: "Already punched out or not punched in.",
      });
      return;
    }
...
const startBreak = async (
  ...
  try {
    ...
    if (!attendance.punchIn || attendance.punchOut?.time) {
      res.status(400).json({
        success: false,
        message: "Must be punched in and not punched out.",
      });
      return;
    }
...
const endBreak = async (
  ...
  try {
    ...
    if (!activeBreak) {
      res
        .status(400)
        .json({ success: false, message: "No active break found." });
      return;
    }

    activeBreak.endTime = new Date();
    activeBreak.duration = calculateWorkMinutes(
...
const getAttendanceReport = async (
  ...
  try {
    ...
    if (startDate || endDate) {
      matchFilter.date = {};
      if (startDate)
        (matchFilter.date as Record<string, unknown>).$gte = startDate;
