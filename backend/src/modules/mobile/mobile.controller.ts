...
import { LocationLog } from "../location/LocationLog.model";
...
const mobileLogin = async (req: Request, res: Response): Promise<void> => {
  try {
          ...
          ? {
              ...
              settings: {
                ...
                requireLocationForPunch:
                  tenant.settings.requireLocationForPunch,
...
const mobilePunchIn = async (
  ...
  try {
    ...
    const attendance =
      existing || new Attendance({ userId, tenantId, date: today });
...
const mobilePunchOut = async (
  ...
  try {
    ...
    if (!attendance?.punchIn) {
      res
        .status(400)
        .json({ success: false, message: "Not punched in today." });
      return;
    }
    ...
    if (attendance.totalWorkMinutes > standardWorkMinutes) {
      attendance.overtimeMinutes =
        attendance.totalWorkMinutes - standardWorkMinutes;
    }
...
const updateWorkMode = async (
  ...
  try {
    ...
    if (!["office", "wfh", "field"].includes(workMode)) {
      res.status(400).json({
        success: false,
        message: "Invalid work mode. Use office, wfh, or field.",
      });
      return;
    }
    ...
    const today = formatDate(new Date());
    await Attendance.findOneAndUpdate({ userId, date: today }, { workMode });
