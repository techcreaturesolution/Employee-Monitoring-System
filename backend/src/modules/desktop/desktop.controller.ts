...
import { Tenant } from "../companies/Company.model";
...
const agentScreenshot = async (
  ...
  try {
    ...
    if (!isCloudinaryConfigured) {
      console.warn(
        "⚠️  Cloudinary not configured. Screenshots will be stored locally."
      );
    } else {
      try {
        const now = new Date();
        ...
        if (cloudinaryResult) {
          ...
          console.error(
            "❌ Cloudinary uploadToCloudinary returned null — falling back to local storage."
          );
        }
      } catch (uploadError) {
        // ⚠️ DO NOT delete the file here — fall back to local storage instead of returning 503
        console.error(
          "❌ Cloudinary upload error (falling back to local):",
    ...
    const screenshot = await Screenshot.create({
      ...
      productivityTag: categorizeActivity(
        activeApp || "",
        windowTitle || "",
        ""
      ),
...
const agentLogActivity = async (
  ...
  try {
    ...
    const docs = activities.map((a: Record<string, unknown>) => ({
      ...
      category:
        a.category && a.category !== "neutral"
          ? a.category
          : categorizeActivity(
              String(a.appName || ""),
              String(a.windowTitle || ""),
              String(a.url || "")
            ),
    }));
    ...
    const existingTimes = new Set(
      existingLogs.map((e) => e.startTime.getTime())
    );
    const uniqueDocs = docs.filter(
      (d) => !existingTimes.has(d.startTime.getTime())
    );
    ...
    if (uniqueDocs.length > 0) {
      ...
      res.status(201).json({
        success: true,
        message: `${uniqueDocs.length} activities logged.`,
        data: { count: uniqueDocs.length },
      });
    } else {
      res.status(200).json({
        success: true,
        message: "No new activities to log.",
        data: { count: 0 },
      });
    }
  } catch (error) {
...
const agentPunchIn = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
    const tenantId = req.user?.tenantId;
...
const getAgentStatus = async (
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data: { isPunchedIn, punchInTime, totalWorkMinutes },
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
const agentPunchOut = async (
  ...
  try {
    ...
    const totalWork = calculateWorkMinutes(
...
const agentSync = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
    const tenantId = req.user?.tenantId;
    ...
    if (Array.isArray(activities) && activities.length > 0) {
      ...
      const docs = activities.map((a: Record<string, unknown>) => {
        ...
        const category =
          validCategories.includes(incomingCat) && incomingCat !== "neutral"
            ? incomingCat
            : categorizeActivity(
                String(a.appName || ""),
                String(a.windowTitle || ""),
    ...
    if (idleTimeMinutes && Number(idleTimeMinutes) > 0) {
      ...
      if (attendance && attendance.punchIn && !attendance.punchOut) {
        attendance.idleMinutes =
          (attendance.idleMinutes || 0) + Number(idleTimeMinutes);
        await attendance.save();
      }
    }
