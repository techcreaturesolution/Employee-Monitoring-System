...
const listProjects = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
const getProjectTimeEntries = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const project = await Project.findOne({ _id: id, tenantId }).populate(
      "timeEntries.userId",
      "name email"
    );
...
const addTimeEntry = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const tenantId = req.user?.tenantId;
    ...
    res
      .status(201)
      .json({ success: true, message: "Time entry added.", data: project });
  } catch (error) {
    res.status(500).json({
      success: false,
